...
uint8_t powerLossCount = 0; // battery voltage less than the voltage threshold
//----------------------------------------------------------------------------
...
volatile static int powerLossSeen = 0; //Flag to indicate if a power loss event has been seen
volatile static int powerDownOLACount = 0;
char powerLossVar[1];
//-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
{
    uint32_t status;
    am_hal_uart_interrupt_status_get(uartH, &status, true);
    am_hal_uart_interrupt_clear(uartH, status);
    // Read or just clear; the interrupt itself wakes the CPU
}
...
//   pin_config(PinName(PIN_PWR_LED), g_AM_HAL_GPIO_OUTPUT);
  ...
  attachInterrupt(PIN_POWER_LOSS, powerLossISR, FALLING);//PIN_POWER_LOSS//PIN_VIN_MONITOR
  powerLossSeen = 0; // Make sure the flag is clear
  powerLossCount = 2;
...
//   printStatements("powerdown count = ", powerLossCount);
  //waitToLog();
...
bool seenSleepCmd() {
  ...
  while (Serial1.available()) {
    ...
    if (have == 5 && memcmp(win, "SLEEP\n", 5) == 0) {
      have = 0;                 // reset window after match
      return true;
    }
  }
  return false;
  ...
  //  }
   ...
   if (triggerPinFlag == true) {
      ...
      if (Serial1.available() >= 5) {  
         if (Serial1.peek() == 'S') {   
            Serial1.readBytes(inbuf, 5); 
         }
      }

      if (strcmp(inbuf, "SLEEP") == 0) {
        am_hal_stimer_int_disable(0xFFFFFFFF);
        am_hal_stimer_int_clear(0xFFFFFFFF);
         powerDownOLA();
      }
...
//       SerialPrint(outputData); //Print to terminal
  ...
  //  Serial1.printf("HELLO\r\n");
   ...
   // printStatements("PL count = ", powerLossCount);
   // sprintf(powerLossVar, "power loss = %s", powerLossSeen);
   // Serial1Print(powerLossVar);
   // Serial1Print(F(" "));
   //Serial1.printf("PIN = %d\r\n", digitalRead(PIN_TRIGGER));
...
void powerLossConfirm(void) {
   ...
   // printStatements("VIN = ", readVIN());
}
//Power Loss ISR
void powerLossISR(void)
{  
   // digitalWrite(PIN_STAT_LED, HIGH);
   Serial1.printf("POWER LOSS ISR\r\n");
   powerLossConfirm();
}

// powerLossSeen = 1;

   // Serial1Print("ISR ACTIVATED\r\n");
   // Serial1Print(F(" "));
//    pinMode(PIN_STAT_LED, OUTPUT);
//   digitalWrite(PIN_STAT_LED, HIGH);
...
//   }
   ...
   if (port == 1) {
      while (Serial1.available()) {
         Serial1.read(); // Read and discard incoming bytes
      }
   }
}
...
//   Serial.write(rtcBuf,7); // for debugging purposes
}
