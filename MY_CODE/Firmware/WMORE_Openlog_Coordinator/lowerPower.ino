void printBattery(void) {
   ...
   int randomVar = 1;
   char powerLoss[5];
   sprintf(powerLoss, "V_IN = %d", randomVar);
   Serial1Print(powerLoss);
   Serial1Print(F(" "));
}
...
void checkBattery(void)
{
  ...
  if (settings.enableLowBatteryDetection == true)
  {
    ...
    if (voltage < settings.lowBatteryThreshold) // Is the voltage low?
    {
      ...
      if (lowBatteryReadings > lowBatteryReadingsLimit) // Have we exceeded the low battery count limit?
      {
        ...
        Serial1Print("POWERING DOWN OLA\r\n");
        Serial1Print(F(" "));
  ...
  //printBattery();
//   printStatements("Power loss val = ", powerLossSeen);
#endif
// sprintf(powerLoss, "power loss = %d", powerLossSeen);
// Serial1Print(powerLoss);
// Serial1Print(F(" "));
  if (powerLossSeen)
    powerDownOLA(); // power down and wait for reset
...
{
  //  Serial1Print("POWER DOWN OLA RUNNING\r\n");
  //  Serial1Print(F(" "));
  ...
  while (1) // Stay in deep sleep until we get reset
  {
    // powerLEDOff();
   //  powerDownOLACount++;
   //  printStatements("powerdown count = ", powerDownOLACount);
